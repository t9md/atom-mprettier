...
class Formatter {
  ...
  isDisabledParser(parser) {
    return getConfig("formatOnSave.disabledParsers").includes(parser)
  }
  ...
  async format(onSave) {
    ...
    if (onSave) {
      ...
      if (!getConfig("formatOnSave.enable")) return
      if (getConfig("formatOnSave.onlyWhenPrettierIsProjectDependencies")) {
    ...
    const {parser} = prettierOptions
    if (!parser) return
    if (onSave) {
      if (this.isDisabledParser(parser)) return
    }
