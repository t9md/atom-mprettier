...
async function findPrettier(editor) {
  ...
  const detectFile = filePath => fs.isFileSync(filePath) && filePath
  ...
  if (getPrettierToUseConfig(which)) {
    libPath = detectInDirectory(path.dirname(filePath))
  }

  if (!libPath && getPrettierToUseConfig((which = "inCurrentProject"))) {
    const dir = atom.project.getDirectories().find(dir => dir.contains(filePath))
    libPath = dir && detectInDirectory(dir.getPath())
  }

  if (!libPath && getPrettierToUseConfig((which = "inAbsolutePath"))) {
    libPath = detectFile(getPrettierToUseConfig("prettierPath"))
  }

  if (!libPath && getPrettierToUseConfig((which = "globallyInstalled"))) {
    if (!NODE_PREFIX) NODE_PREFIX = await getNodePrefixPath()
    if (NODE_PREFIX) {
      // Windows || non Windows
      libPath = detectInDirectory(NODE_PREFIX) || detectInDirectory(NODE_PREFIX, "lib")
    }
  }

  if (!libPath && getPrettierToUseConfig((which = "bundled"))) {
    libPath = detectInDirectory(path.dirname(__dirname))
  }
