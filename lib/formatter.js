...
async function detectPrettierPath(editor) {
  ...
  if (getConfig(source)) {
    const libPath = detectFile(getConfig("prettierInAbsolutePath"))
    if (libPath) return {libPath, source}
  }
...
function detectParser(editor) {
  ...
  for (const parserName of Object.keys(scopesByParserName)) {
    if (scopesByParserName[parserName].includes(scopeName)) {
      return parserName === "javascript" ? getConfig("javascriptParser") : parserName
    }
  }
}
...
class Formatter {
  ...
  getPrettierOptions(prettier) {
    const options = prettier.resolveConfig.sync(this.editor.getPath()) || getConfig("prettierOptions")
    return Object.assign(options, {parser: detectParser(this.editor)})
  }

  isDisabledParser(parser, onSave) {
    return (
      (onSave && getConfig("disabledParsersOnFormatOnSave").includes(parser)) ||
      getConfig("disabledParsers").includes(parser)
    )
  }

  shouldFormat(parser, onSave) {
    return parser && !this.isDisabledParser(parser, onSave)
  }
  ...
  async format(onSave) {
    ...
    if (this.isDisabledFile(editor.getPath())) return
    if (onSave && !getConfig("formatOnSave")) return
    if (getConfig("formatOnlyWhenPrettierIsProjectDependencies")) {
      const cwd = path.dirname(editor.getPath())
      if (!hasClosestPackageJsonHasPackage(cwd, "prettier")) return
    }
    ...
    if (!this.shouldFormat(prettierOptions.parser, onSave)) return

    if (getConfig("debug")) {
      const debugInfo = this.buildDebuggInfo({prettier, libPath, source, prettierOptions})
  ...
  buildDebuggInfo({prettier, libPath, source, prettierOptions}) {
    return {
      atomVersion: atom.getVersion(),
      platform: process.platform,
      mprettierVersion: require(path.join(path.dirname(__dirname), "package.json")).version,
      editorGrammarScope: this.editor.getGrammar().scopeName,
      prettierVersion: prettier.version,
      prettierPath: libPath,
      prettierSource: source,
      prettierOptions: prettierOptions || (prettier ? this.getPrettierOptions(prettier) : null),
  ...
  async clipDebugInfo() {
    ...
    const prettier = libPath ? require(libPath) : undefined
    const text = require("util").inspect(this.buildDebuggInfo({prettier, libPath, source}), {depth: null})
